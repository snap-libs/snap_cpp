...
std::string PhonologyJa::apply_rules(const std::string& text, 
    ...
    // Handle script="hiragana"
    if (script == "hiragana") {
        std::string hira = "";
        std::vector<uint32_t> cps = utf8_to_codepoints_ja(kata);
        for (uint32_t cp : cps) {
            if (cp >= 0x30A1 && cp <= 0x30F6) {
                uint32_t hira_cp = cp - 0x60;
                hira += codepoint_to_utf8_ja(hira_cp);
            } else {
                hira += codepoint_to_utf8_ja(cp);
            }
        }
        kata = hira;
    }
    ...
    // Handle pitch_accent
    if (pitch_accent && !out_accent_overrides.empty()) {
        std::string accent_info = " [";
        bool first = true;
        for (const auto& [word, type] : out_accent_overrides) {
            if (!first) accent_info += " ";
            accent_info += word + ":" + std::to_string(type);
            first = false;
        }
        accent_info += "]";
        kata += accent_info;
    ...
    // Handle to_ipa
    if (to_ipa) {
        static const std::unordered_map<std::string, std::string> KATA_IPA = {
            {"ア", "a"}, {"イ", "i"}, {"ウ", "ɯ"}, {"エ", "e"}, {"オ", "o"},
            ...
            {"シャ", "ɕa"}, {"シュ", "ɕɯ"}, {"ショ", "ɕo"},
            {"チャ", "tɕa"}, {"チュ", "tɕɯ"}, {"チョ", "tɕo"},
            {"ニャ", "ɲa"}, {"ニュ", "ɲɯ"}, {"ニョ", "ɲo"},
            {"ヒャ", "çja"}, {"ヒュ", "çjɯ"}, {"ヒョ", "çjo"},
        ...
        std::vector<uint32_t> cps = utf8_to_codepoints_ja(kata);
        for (size_t i = 0; i < cps.size(); ++i) {
            std::string ch = codepoint_to_utf8_ja(cps[i]);
            if (KATA_IPA.count(ch)) {
                ipa_out += KATA_IPA.at(ch);
            } else {
                ipa_out += ch;
            }
        }
