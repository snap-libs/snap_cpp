...
public:
    ...
    struct BatchOutput {
        std::vector<float> hidden_states;  // flattened [batch_size, max_seq_len, hidden_dim]
        std::vector<int> seq_lens;         // actual token count per sentence
        int batch_size = 0;
        int max_seq_len = 0;
        int hidden_dim = 768;
        std::vector<std::vector<std::pair<size_t, size_t>>> offsets; // byte offsets per sentence
    };

    Output get_hidden_states(const std::string& text);
    BatchOutput get_hidden_states_batch(const std::vector<std::string>& texts);
