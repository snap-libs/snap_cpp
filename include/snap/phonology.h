...
public:
    ...
    /// Extract prosodic pauses using 2-pass algorithm
    std::vector<PauseInfo> extract_prosodic_pauses(const std::string& text, const std::vector<MorphItem>& morphemes);

    /// Format phonology text with W3C SSML <break> tags
    std::string apply_ssml_breaks(const std::string& phonology_text, const std::vector<PauseInfo>& pauses);

    /// Apply all Korean phonology rules sequentially and return the phonetic transcription or IPA string
    std::string apply_rules(const std::string& text, const SnapResult& result, const std::string& vowel_length_style = "none", bool to_ipa = false, bool to_ssml = false);
