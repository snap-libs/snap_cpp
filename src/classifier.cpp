...
bool ContextClassifier::init(const std::string& weights_dir,
    ...
    // 1. Establish single SNAP_HOME anchor (100% identical logic to Python get_snap_home)
    std::string snap_home = "";

    // Step 1: Check override_path (weights_dir)
    if (!weights_dir.empty()) {
        std::string p = weights_dir;
        try {
            std::filesystem::path fs_p(weights_dir);
            if (std::filesystem::exists(fs_p)) {
                p = std::filesystem::canonical(fs_p).string();
            } else {
                p = std::filesystem::absolute(fs_p).string();
            }
        } catch (...) {
            p = weights_dir;
        }
        ...
        size_t sep1 = p.find_last_of("/\\");
        std::string name1 = (sep1 != std::string::npos) ? p.substr(sep1 + 1) : p;

        if (name1 == "models" || name1 == "weights") {
            snap_home = (sep1 != std::string::npos) ? p.substr(0, sep1) : ".";
        } else if (name1 == "ko" || name1 == "en" || name1 == "ja") {
            std::string parent1 = (sep1 != std::string::npos) ? p.substr(0, sep1) : ".";
            size_t sep2 = parent1.find_last_of("/\\");
            std::string name2 = (sep2 != std::string::npos) ? parent1.substr(sep2 + 1) : parent1;
            if (name2 == "models" || name2 == "weights") {
                snap_home = (sep2 != std::string::npos) ? parent1.substr(0, sep2) : ".";
            } else {
                snap_home = p;
            }
        } else {
    ...
    // Step 2: Check environment variables if snap_home is still empty
    if (snap_home.empty()) {
    ...
    while (!snap_home.empty() && (snap_home.back() == '/' || snap_home.back() == '\\')) {
        snap_home.pop_back();
    }

    // Normalize snap_home path using std::filesystem
    try {
        std::filesystem::path fs_home(snap_home);
        if (std::filesystem::exists(fs_home)) {
            snap_home = std::filesystem::canonical(fs_home).string();
        }
    } catch (...) {}

    // Step 3: Derive language model directory strictly relative to snap_home anchor (<snap_home>/models/<clean_lang>)
    std::string clean_lang = lang;
    std::transform(clean_lang.begin(), clean_lang.end(), clean_lang.begin(), ::tolower);
    if (clean_lang == "kr") clean_lang = "ko";
    if (clean_lang == "jp") clean_lang = "ja";

    // Strict single target path without any fallback magic
    std::string ld = snap_home + "/models/" + clean_lang;

    // Load snap_config.json first (Strict single path enforcement)
    ...
    if (cfg.is_discarded() || !cfg.is_object()) {
        std::cerr << "[SNAP Strict Policy Error] snap_config.json을 찾을 수 없습니다!\n"
                  ...
                  << "  - 탐색 경로: " << (ld + "/snap_config.json") << "\n"
                  << "  - 올바른 규격 위치: <SNAP_HOME>/models/" << clean_lang << "/snap_config.json\n" << std::endl;
        return false;
    }
...
SnapResult ContextClassifier::process(const std::string& text, const std::string& config_file) {
    SnapGlobalConfig global_cfg = SnapGlobalConfig::LoadOrDefault(config_file);
    ...
    if (impl_->language == "ko") {
        ...
        std::string v_length_style = global_cfg.ko.vowel_length ? "colon" : "none";
        result.phonology = impl_->phonology_kr.apply_rules(
            result.normalized_text, result, v_length_style, global_cfg.ko.to_ipa
        );
    } else if (impl_->language == "ja") {
        result.phonology = impl_->phonology_ja.apply_rules(
            text, result.annotations, result.morphemes, result.accent_overrides,
            global_cfg.ja.script, global_cfg.ja.pitch_accent, global_cfg.ja.to_ipa
        );
    } else if (impl_->language == "en") {
        result.phonology = impl_->phonology_en.apply_rules(
            text, result, impl_->targets_ipa_map, global_cfg.en.to_ipa
        );
    }
