...
음성합성(TTS) 시스템은 일반적으로 **텍스트 정규화(TN) $\rightarrow$ 음운 변환(G2P) $\rightarrow$ 음향 모델(Acoustic Model) $\rightarrow$ 보코더(Vocoder)**의 순차 파이프라인 구조를 따릅니다.  
...
* **1. 규칙 및 FST(유한상태변환기) 기반 방식 (g2pK, Thrax 등)**:
...
* **단독 구동 모드 (Standalone Mode)**: 약 120MB의 가벼운 메모리 점유율과 CPU 평균 ~44ms, GPU 11~14ms 수준의 처리 속도로 일반적인 실시간 TTS 파이프라인에 독립적으로 연동됩니다.
...
* **한국어 표준 발음법 전수 지원**:
  * 국립국어원 표준 발음법(제1항~제30항)에 명시된 자음동화(비음화/유음화), 구개음화, 격음화, 된소리되기(경음화), 연음 규칙, 음절의 끝소리(중화), `ㄺ/ㄼ` 특례 발음, 자모 이름 치환(ㄱ $\rightarrow$ 기역) 등 복잡한 한국어 음운 변환을 누락 없이 적용합니다.
* **품사 및 문맥에 따른 발음 변별**:
...
2. **고성능 C++ Native SDK 및 다양한 개발 환경 지원**:
   * C-Linkage 기반의 독립적인 네이티브 SDK(`snap_cpp`)와 Python 패키지(`snap_py`)를 제공하여 C/C++, Rust, Go, Python, Node.js 등 다양한 언어 및 서비스 환경에 손쉽게 통합할 수 있습니다.
   * 타깃 시스템의 하드웨어 환경에 맞춰 가속 실행 엔진(Execution Provider: `CUDA`, `DirectML`, `CoreML`, `CPU`)을 자동으로 감지하고 최적화합니다.
...
### 1) 다국어 (한·일·영) 지원 현황 및 성숙도

한·일·영 3개 국어 모두 신경망 Probing Head 및 전처리 파이프라인이 구축되어 있으며, 언어별 특성에 맞춘 고도화가 진행되어 있습니다:

* **한국어 (KO) — 상용화 완성 단계 (★★★★★)**:
  * 1만 건 규모의 표준 골든 베이스라인 전수 검증 완료.
  * 표준 발음법 30개 항, 수사/단위 문맥 분별, 혼용 영단어 낭독, 운율 쉼 자동 지정 등 엔드투엔드 상용 서비스 수준 완성.
* **일본어 (JA) — 엔진 완성 및 사전/룰 고도화 단계 (★★★☆☆)**:
  * 446개 클래스 한자 음/훈독 다의어 판별 Head 탑재 (`1日`: ついたち vs いちにち / `人気`: にんき vs ひとけ).
  * 기본 G2P 파이프라인 및 고유명사 사전 연동 완료, 실사용 도메인별 세부 사전 및 예외 규칙 보강 단계.
* **영어 (EN) — 엔진 완성 및 사전/룰 고도화 단계 (★★★☆☆)**:
  * 173개 클래스 Heteronym(동철이음이의어) 문맥 발음 판별 Head 탑재 (`live` [/lɪv/] vs [/laɪv/], `read` [/riːd/] vs [/rɛd/]).
  * CMUdict 기반 발음 변환 및 시맨틱 기호 분별 완료, 세부 특수 도메인 약어/예외 룰 보강 단계.

### 2) 다양한 음성합성(TTS) 엔진과의 연동을 위한 유연한 출력 옵션
* **W3C 표준 SSML 지원**: `<speak>`, `<break strength="..."/>` 등의 표준 마크업을 출력하여 상용 클라우드 TTS(Amazon Polly, Google Cloud TTS, Azure TTS 등)와 손쉽게 호환.
* **딥러닝 TTS 모델별 맞춤형 포맷 제공**: MeloTTS, F5-TTS, StyleTTS2, Piper, CosyVoice 등 다양한 음향 모델 구조에 맞춰 표준 한글 발음열(G2P), 국제 음성 기호(IPA), 음절 분절 데이터 등 필요한 포맷을 유연하게 추출하여 연동 가능.
* **BERT 임베딩 및 정렬 데이터 연계**: BERT 내장형 음향 모델 연동 시 계산된 Hidden States 및 단어-음소 정렬 메타데이터를 직접 전달하여 파이프라인의 중복 연산 방지.
...
* **ITN(역텍스트 정규화)이란?**  
  ...
  * **금액/통화**: `"이만 삼천오백원"` $\rightarrow$ **`23,500원`** / `"백오십 달러"` $\rightarrow$ **`$150`**
  * **날짜/시간**: `"이천이십육년 팔월 이십사일"` $\rightarrow$ **`2026년 8월 24일`** / `"오후 세시 십분"` $\rightarrow$ **`오후 3:10`**
  * **수량/단위**: `"삼점오 킬로그램"` $\rightarrow$ **`3.5kg`** / `"이십오 퍼센트"` $\rightarrow$ **`25%`**
  * **일본어 예시**: `"にまんさんぜんえん"` $\rightarrow$ **`23,000円`** / `"ごがついつか"` $\rightarrow$ **`5月5日`**
...
* **한국어·일본어 ITN 지원 (공개 예정)**:
  * 한국어 및 일본어 대상의 수사, 날짜, 시간, 통화, 단위 역정규화 파이프라인을 구축 완료했으며, 조만간 정식 공개될 예정입니다.
