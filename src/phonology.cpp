...
bool PhonologyKr::init(const std::string& weights_dir) {
    ...
    // Load dict_loanwords.json (SNAP 정식 표준 규격: models/ko/dict_loanwords.json)
    {
        std::string dict_path = weights_dir + "/models/ko/dict_loanwords.json";
        std::ifstream ifs(dict_path, std::ios::binary);
        if (!ifs.is_open()) {
            std::cerr << "\n[SNAP Strict Policy Error] 필수 외래어 사전 자원(dict_loanwords.json)을 찾을 수 없습니다!\n"
                      << "  - 표준 탐색 경로: " << dict_path << "\n"
                      << "  - 조치 방법: 필수 사전 파일(dict_loanwords.json)을 models/ko/ 디렉토리에 배치하십시오.\n" << std::endl;
            return false;
        }
        try {
            nlohmann::json j = nlohmann::json::parse(ifs, nullptr, false);
            if (j.is_discarded() || !j.is_object()) {
                std::cerr << "[SNAP Strict Policy Error] dict_loanwords.json 파싱 실패!" << std::endl;
                return false;
            }
            ...
            std::cerr << "[SNAP Strict Policy Error] dict_loanwords.json 로딩 예외: " << e.what() << std::endl;
            return false;
        }
    }
...
bool PhonologyKr::is_loanword_span(const std::vector<HangulTuple>& tuples, int start_idx, int nxt_idx) {
    ...
    int end = start_idx;
    while (end < static_cast<int>(tuples.size()) - 1 && tuples[end + 1].type == 'H' && end + 1 < nxt_idx) {
        end++;
    }
...
void PhonologyKr::apply_n_addition(std::vector<HangulTuple>& tuples) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (curr.cho == "ㅅ" && curr.jung == "ㅣ" && curr.jong == "ㅂ" && nxt.cho == "ㅇ" && nxt.jung == "ㅠ") {
            nxt.cho = "ㄴ";
        }
    }
}
...
void PhonologyKr::apply_aspiration_and_h_drop(std::vector<HangulTuple>& tuples) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (cho == "ㅎ") {
            ...
            if (has_space) {
                continue; // 띄어쓰기 너머 어절 간 격음화 스킵 (제조업 혁신 -> [제조업 혁신])
            }
            if (jong == "ㄱ" || jong == "ㄺ") {
                curr.jong = (jong == "ㄺ") ? "ㄹ" : "";
                nxt.cho = "ㅋ";
...
void PhonologyKr::apply_neutralization(std::vector<HangulTuple>& tuples) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // 관형사 '첫' 예외: 뒤에 거센소리가 오면 'ㅅ' 중화 스킵 (첫 차량 -> 첫 차량)
        if (jong == "ㅅ" && curr.cho == "ㅊ" && curr.jung == "ㅓ") {
            int nxt_idx = get_next_hangul_idx(tuples, i);
            if (nxt_idx != -1 && tuples[nxt_idx].type == 'H') {
                const std::string& next_cho = tuples[nxt_idx].cho;
                if (next_cho == "ㅊ" || next_cho == "ㅋ" || next_cho == "ㅌ" || next_cho == "ㅍ") {
                    continue;
                }
            }
        }
...
void PhonologyKr::apply_assimilation(std::vector<HangulTuple>& tuples) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        const std::string& pos = curr.meta.pos;
        bool is_ending = (pos == "EC" || pos == "EF" || pos == "ETM" || pos == "EP");
        if (has_space || is_ending) continue;
...
std::string PhonologyKr::hangul_to_ipa(const std::vector<HangulTuple>& tuples, bool include_vowel_length) {
    static const std::unordered_map<std::string, std::string> CHO_IPA = {
        {"ㄱ", "k"}, {"ㄲ", "k͈"}, {"ㄴ", "n"}, {"ㄷ", "t"}, {"ㄸ", "t͈"}, {"ㄹ", "ɾ"},
        {"ㅁ", "m"}, {"ㅂ", "p"}, {"ㅃ", "p͈"}, {"ㅅ", "s"}, {"ㅆ", "s͈"}, {"ㅇ", ""},
        {"ㅈ", "tɕ"}, {"ㅉ", "tɕ͈"}, {"ㅊ", "tɕʰ"}, {"ㅋ", "kʰ"}, {"ㅌ", "tʰ"}, {"ㅍ", "pʰ"}, {"ㅎ", "h"}
    };

    static const std::unordered_map<std::string, std::string> JUNG_IPA = {
        {"ㅏ", "a"}, {"ㅐ", "ɛ"}, {"ㅑ", "ja"}, {"ㅒ", "jɛ"}, {"ㅓ", "ʌ"}, {"ㅔ", "e"},
        {"ㅕ", "jʌ"}, {"ㅖ", "je"}, {"ㅗ", "o"}, {"ㅘ", "wa"}, {"ㅙ", "wɛ"}, {"ㅚ", "ø"},
        {"ㅛ", "jo"}, {"ㅜ", "u"}, {"ㅝ", "wʌ"}, {"ㅞ", "we"}, {"ㅟ", "ɥi"}, {"ㅠ", "ju"},
        {"ㅡ", "ɯ"}, {"ㅢ", "ɰi"}, {"ㅣ", "i"}
    };

    static const std::unordered_map<std::string, std::string> JONG_IPA = {
        {"", ""}, {"ㄱ", "k̚"}, {"ㄲ", "k̚"}, {"ㄳ", "k̚"}, {"ㄴ", "n"}, {"ㄵ", "n"}, {"ㄶ", "n"},
        {"ㄷ", "t̚"}, {"ㄹ", "l"}, {"ㄺ", "k̚"}, {"ㄻ", "m"}, {"ㄼ", "l"}, {"ㄽ", "l"}, {"ㄾ", "l"}, {"ㄿ", "p̚"}, {"ㅀ", "l"},
    ...
    std::string ipa_str = "";
    for (size_t i = 0; i < tuples.size(); ++i) {
        const auto& item = tuples[i];
        if (item.type != 'H') {
            ipa_str += item.cho;
            continue;
        }

        std::string cho_ipa = CHO_IPA.count(item.cho) ? CHO_IPA.at(item.cho) : "";
        std::string jung_ipa = JUNG_IPA.count(item.jung) ? JUNG_IPA.at(item.jung) : "";
        std::string jong_ipa = JONG_IPA.count(item.jong) ? JONG_IPA.at(item.jong) : "";

        // Liquid assimilation: l + ɾ -> ll
        if (i > 0 && tuples[i - 1].type == 'H' && tuples[i - 1].jong == "ㄹ" && item.cho == "ㄹ") {
            cho_ipa = "l";
        }

        if (include_vowel_length && item.meta.is_long) {
            jung_ipa += "ː";
        }

        ipa_str += cho_ipa + jung_ipa + jong_ipa;
    }

    return "[" + ipa_str + "]";
}

std::string PhonologyKr::apply_rules(const std::string& text, const SnapResult& result, const std::string& vowel_length_style, bool to_ipa) {
    try {
        ...
        if (to_ipa) {
            return hangul_to_ipa(tuples, true);
        }
