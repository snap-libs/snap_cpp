...
HF_ZIP_URL="https://huggingface.co/softguy777/snap-weights/archive/main.tar.gz"
...
LANG_LOWER=$(echo "$LANG_PARAM" | tr '[:upper:]' '[:lower:]')

if [ -z "$LANG_LOWER" ] || [ "$LANG_LOWER" = "all" ] || [ "$LANG_LOWER" = "*" ]; then
    TARGET_LANGS=("ko" "ja" "en")
    IS_ALL_LANGS=true
else
    IFS=',' read -ra ADDR <<< "$LANG_LOWER"
    for i in "${ADDR[@]}"; do
        trimmed=$(echo "$i" | xargs)
        if [[ " ${ALL_LANGS[*]} " =~ " ${trimmed} " ]]; then
            TARGET_LANGS+=("$trimmed")
        fi
    done
    if [ ${#TARGET_LANGS[@]} -eq 0 ]; then
        echo " [WARNING] No valid language specified in '$LANG_PARAM'. Defaulting to ALL languages."
        TARGET_LANGS=("ko" "ja" "en")
        IS_ALL_LANGS=true
    elif [ ${#TARGET_LANGS[@]} -eq ${#ALL_LANGS[@]} ]; then
        IS_ALL_LANGS=true
    else
        IS_ALL_LANGS=false
    fi
...
prune_unselected_languages() {
    local models_path="$1"
    for l in "${ALL_LANGS[@]}"; do
        if [[ ! " ${TARGET_LANGS[*]} " =~ " ${l} " ]]; then
            local dir_to_prune="${models_path}/${l}"
            if [ -d "$dir_to_prune" ]; then
                echo " -> Pruning unselected language directory: ${dir_to_prune}"
                rm -rf "$dir_to_prune"
            fi
        fi
    done
...
if [ "$AUTO_CONFIRM" = false ]; then
    read -p "Do you want to set '${ABS_TARGET_DIR}' as SNAP Root and download models (${TARGET_LANGS[*]})? (y/N): " CONFIRM
    case "$CONFIRM" in
        [yY][eE][sS]|[yY])
            ;;
        *)
...
echo ""
echo "[1/2] Downloading SNAP models & lexicons (${TARGET_LANGS[*]}) into: ${MODELS_DIR}"
...
if command -v git >/dev/null 2>&1; then
    ...
    if [ "$IS_ALL_LANGS" = true ]; then
        git clone --depth 1 "$HF_REPO_URL" "$MODELS_DIR"
    else
        echo " -> Pinpoint downloading selected language(s) [${TARGET_LANGS[*]}] via Git sparse-checkout..."
        git clone --depth 1 --no-checkout "$HF_REPO_URL" "$MODELS_DIR"
        if [ -d "$MODELS_DIR" ]; then
            (
                cd "$MODELS_DIR"
                git sparse-checkout init --cone || true
                git sparse-checkout set "${TARGET_LANGS[@]}" || true
                git checkout || true
            )
            prune_unselected_languages "$MODELS_DIR"
        fi
    fi
else
...
if [ -f "${MODELS_DIR}/manifest.json" ] || [ -f "${MODELS_DIR}/snap_config.json" ] || [ -d "${MODELS_DIR}/ko" ] || [ -d "${MODELS_DIR}/ja" ] || [ -d "${MODELS_DIR}/en" ]; then
...
echo "  - Models    : ${MODELS_DIR} (${TARGET_LANGS[*]})"
