...
bool TextNormalizeKr::init(const std::string& weights_dir) {
    ...
    // Load dict_eng_brand.json & dict_eng_custom.json
    for (const auto& fn : {std::string("dict_eng_brand.json"), std::string("dict_eng_custom.json")}) {
        std::ifstream ifs(ko_dir + fn, std::ios::binary);
        if (ifs.is_open()) {
            json j = json::parse(ifs, nullptr, false);
            if (!j.is_discarded() && j.is_object()) {
                for (auto& [k, v] : j.items()) {
...
static std::string num_label_to_kor(int64_t n, const std::string& label, const std::string& after) {
    if (label == "native") {
        return to_native_korean_count(static_cast<int>(n));
    }
    if (label == "sino") {
        return num2words_ko(n);
    }
    // No label: check native and sino unit lists
    if (n >= 100) {
        return num2words_ko(n);
    }
...
std::vector<NormalizedSpan> TextNormalizeKr::scan(
    ...
    // 0.1 Corporate symbol normalization — ㈜ / (주) -> 주식회사
    {
        std::regex corp_re(R"(㈜|\(주\))");
        auto it = std::sregex_iterator(text.begin(), text.end(), corp_re);
        auto end = std::sregex_iterator();
        for (; it != end; ++it) {
            add_span(it->position(), it->position() + it->length(), u8"주식회사 ");
        }
    }

    // 0.2 Range tilde normalization — 2250~2540 -> 2250 에서 2540
    {
        std::regex range_re(R"((\d)\s*[~∼]\s*(\d))");
        auto it = std::sregex_iterator(text.begin(), text.end(), range_re);
        auto end = std::sregex_iterator();
        for (; it != end; ++it) {
            std::smatch m = *it;
            size_t start_pos = m.position(0) + m[1].length();
            size_t end_pos = m.position(0) + m[0].length() - m[2].length();
            add_span(start_pos, end_pos, u8"에서 ");
        }
    }

    // 0.5 Colon Patterns for Time/Score/Ratio/Verse
    {
        std::regex time_re(R"((?:^|[^\d])([01]?\d|2[0-3]):([0-5]\d)(?!\d))");
        auto it = std::sregex_iterator(text.begin(), text.end(), time_re);
        auto end = std::sregex_iterator();
        for (; it != end; ++it) {
            std::smatch match = *it;
            size_t match_pos = match.position(0);
            size_t match_len = match.length(0);
            if (!match.str(0).empty() && !std::isdigit((unsigned char)match.str(0)[0]) && match.str(0)[0] != '.') {
                match_pos += 1;
                match_len -= 1;
            }
            int h = std::stoi(match[1].str());
            int m_val = std::stoi(match[2].str());
            std::string h_kor = (h >= 1 && h <= 12) ? to_native_korean(h) : num2words_ko(h);
            std::string m_kor = (m_val == 0) ? u8" 정각" : (u8" " + num2words_ko(m_val) + u8" 분");
            add_span(match_pos, match_pos + match_len, h_kor + u8" 시" + m_kor);
        }
    }
    ...
    for (const auto& [unit, kor_unit] : suffix_unit_map()) {
        ...
        // 4b. Integer + unit
        {
            ...
            for (; it != end; ++it) {
                ...
                std::string after_sub = text.substr(end_pos);
                size_t first_non_space = after_sub.find_first_not_of(" \t");
                std::string actual_repl = num2words_ko(val) + kor_unit;
                if ((unit == u8"℃" || unit == "°C") && first_non_space != std::string::npos &&
                    after_sub.compare(first_non_space, 3, u8"도") == 0) {
                    actual_repl = num2words_ko(val);
                }
                add_span(it->position(), end_pos, actual_repl);
            }
        }
    }

    // 5.4 Dot-delimited date patterns (2022.3.25, 2022. 3. 25., 2026.12)
    {
        std::regex re3(R"((?:^|[^\d])(19\d{2}|20\d{2}|\d{2})\.\s*(1[0-2]|0?[1-9])\.\s*(3[01]|[12]\d|0?[1-9])\.?(?!\d))");
        std::sregex_iterator it(text.begin(), text.end(), re3);
        std::sregex_iterator end;
        for (; it != end; ++it) {
            std::smatch match = *it;
            size_t match_pos = match.position(0);
            ...
            int m_val = std::stoi(match[2].str());
            int d_val = std::stoi(match[3].str());
            int full_y = (y > 100) ? y : ((y <= 30) ? 2000 + y : 1900 + y);
            std::string y_str = num2words_ko(full_y) + u8"년 ";
            std::string m_str = num2words_ko(m_val) + u8"월 ";
            std::string d_str = num2words_ko(d_val) + u8"일";
            add_span(match_pos, match_pos + match_len, y_str + m_str + d_str);
        }

        std::regex re2(R"((?:^|[^\d])(19\d{2}|20\d{2})\.\s*(1[0-2]|0?[1-9])\.?(?!\d|\.\d|달러|포인트|원|kg|g|m|cm|mm|%|퍼센트))");
        std::sregex_iterator it2(text.begin(), text.end(), re2);
        for (; it2 != end; ++it2) {
            ...
            int y = std::stoi(match[1].str());
            int m_val = std::stoi(match[2].str());
            std::string y_str = num2words_ko(y) + u8"년 ";
            std::string m_str = num2words_ko(m_val) + u8"월";
            add_span(match_pos, match_pos + match_len, y_str + m_str);
        }
    }

    // 5.5 Decimal numbers with commas (e.g. 2,650.32 -> 이천육백오십쩜삼이)
    {
...
std::string TextNormalizeKr::apply_spans(const std::string& text,
    ...
    std::regex double_space_re(R"( +)");
    result = std::regex_replace(result, double_space_re, " ");
...
std::string TextNormalizeKr::english_word_to_korean(const std::string& word) const {
    ...
    // ②.5 CamelCase split fallback
    bool has_inner_upper = false;
    for (size_t k = 1; k < word.length(); ++k) {
        if (isupper((unsigned char)word[k])) { has_inner_upper = true; break; }
    }
    if (has_inner_upper) {
        std::regex camel_re(R"([A-Z][a-z]+|[A-Z]+(?=[A-Z][a-z]|\b)|[a-z]+)");
        auto it = std::sregex_iterator(word.begin(), word.end(), camel_re);
        auto end = std::sregex_iterator();
        std::string camel_res;
        int parts_count = 0;
        for (; it != end; ++it) {
            parts_count++;
            std::string part = it->str();
            auto e_it = english_dictionary_exact_.find(part);
            if (e_it != english_dictionary_exact_.end()) {
                camel_res += e_it->second;
            } else {
                auto u_it = english_dictionary_.find(to_upper(part));
                if (u_it != english_dictionary_.end()) {
                    camel_res += u_it->second;
                } else {
                    for (char c : part) {
                        auto a_it = eng_alphabet().find((char)toupper(c));
                        if (a_it != eng_alphabet().end()) {
                            camel_res += a_it->second;
                        } else {
                            camel_res += c;
                        }
                    }
