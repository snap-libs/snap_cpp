...
echo ========================================================
echo   SNAP TTS Frontend Windows Quick Test - Korean (One-Click)
echo ========================================================
echo.

:: 1. Define paths and URLs
set "ZIP_URL=https://huggingface.co/softguy777/snap-weights/resolve/main/quickstart/win-x64-ko-int8-quickstart.zip"
set "ZIP_FILE=win-x64-ko-int8-quickstart.zip"
set "TARGET_DIR=..\snap\weights"

:: 2. Download ZIP file using PowerShell
echo [1/3] 허깅페이스에서 한국어 테스트 패키지 다운로드 중...
echo (약 79MB 크기이며 네트워크 환경에 따라 시간이 약간 소요될 수 있습니다.)
...
echo [입력 문장]: 안과에 갔다.
echo.
echo 안과에 갔다. | "%TARGET_DIR%\test_e2e.exe" "%TARGET_DIR%" ko
echo --------------------------------------------------------
echo.
